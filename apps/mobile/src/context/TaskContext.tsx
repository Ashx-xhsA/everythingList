...
export const TaskProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const completeTask = (id: string) => {
    ...
    const newTasks = completeTaskState(id, tasks as any) as Task[];
    setTasks(newTasks);
  ...
  const firePage = (pageIndex: number) => {
    const newTasks = dismissPageTasks(pageIndex, tasks as any) as Task[];
    setTasks(newTasks);
    if (userId) {
  ...
  const nextPage = () => {
    ...
    if (!actionTakenOnCurrentPage && !isLastPage) {
      newTasks = dismissPageTasks(currentPageIndex, tasks as any) as Task[];
      setTasks(newTasks);
      if (userId) {
